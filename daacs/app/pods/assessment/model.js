...
import {
    ...
    SCORING_LIGHTSIDE,
    SCORING_BERT,
} from "daacs/constants/assessment/scoring-types";
import AssessmentTypes, {
    TYPE_MULTIPLE_CHOICE,
    TYPE_CAT,
    TYPE_LIKERT,
    TYPE_WRITING_PROMPT,
} from "daacs/constants/assessment/types";

const {
    get,
    computed,
    computed: { alias, equal, and, or },
} = Ember;
...
export default Model.extend({
    ...
    isMultipleChoice: equal(
        "assessmentType",
        get(TYPE_MULTIPLE_CHOICE, "value")
    ),
    ...
    itemGroupItems: computed("itemGroups.@each.items", function () {
        const sortedGroups = get(this, "itemGroups").toArray();
        return sortedGroups;
    }),
    largestItemGroup: computed("itemGroupItems.@each.length", function () {
        const sortedGroups = get(this, "itemGroupItems").sortBy("length");
        return get(sortedGroups, "lastObject");
    }),
    lowerCaseCategory: lowerCase("assessmentCategory"),
    numUserCompletionsText: t("admin.numCompletions", {

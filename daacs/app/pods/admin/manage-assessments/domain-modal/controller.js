...
export default Ember.Controller.extend(ModalDialog, EmberValidations, {
    ...
    async uploadModelFile(files) {
        if (!files || !files[0] || isEmpty(files[0])) {
            return;
        }

        try {
            const scoringType = get(
                this,
                "assessment.scoringType"
            ).toLowerCase();
            const uploader = Uploader.create({
                url: `${Config.RESTAPI}/assessments/upload-${scoringType}-models`,
                ajaxSettings: {
                    headers: {
            ...
            set(
                this,
                `model.${scoringType}ModelFilename`,
                get(response, "fileName")
            );
            get(this, "notify").success(
                get(this, "i18n").t(`admin.${scoringType}.uploadSuccess`)
            );
            return response;
        } catch (error) {
            const errCode =
                get(error, "responseJSON.errors.firstObject.code") ||
                "serverError.msg";
            set(this, "isUploading", false);
    ...
    actions: {
        save() {
            ...
            const isSubDomain = !isNone(
                assessmentDomains.find((d) =>
                    get(d, "subDomains").toArray().includes(model)
                )
            );
            const inAssessment =
                isSubDomain || assessmentDomains.toArray().includes(model);
        ...
        updateParent(newParentId) {
            const assessmentDomains = get(this, "assessment.domains");
            const newParent =
                newParentId && assessmentDomains.findBy("id", newParentId);

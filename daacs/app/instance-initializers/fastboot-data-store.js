  ...
  if (fastboot && fastboot.get('isFastBoot')) {
      ...
      shoebox.put('ember-data-store', {
        get records() {
            ...
            let types = Object.keys(store.typeMaps).map(k => {
            ...
            types.forEach(type => {
                ...
                data[type] = store.peekAll(type).toArray().reduce((a,b) => a.concat(b), [])
                  .map(record => {
                      // Serialize the record and recieve json
                      return serializer.serialize(record._createSnapshot(), { includeId: true});
                  });
            });
